...
pub(crate) struct NodeShared<D: Direction> {
    ...
    pub(in crate::node) started: AtomicBool,
    ...
    pub(in crate::node) generation: AtomicU64,
    ...
    pub(in crate::node) slot_state: AtomicU8,
    pub(in crate::node) slot: UnsafeCell<Option<DeviceSwap<D>>>,
}
...
impl<D: Direction> NodeShared<D> {
    pub(in crate::node) fn new() -> Self {
        Self {
            ...
            slot_state: AtomicU8::new(SLOT_EMPTY),
    ...
    pub(in crate::node) fn alive_token(&self) -> Arc<AtomicBool> {
        self.alive.clone()
    }
    ...
    pub(in crate::node) fn swap_ready(&self) -> bool {
        self.slot_state.load(Ordering::Relaxed) == SLOT_FULL
    }
...
pub(in crate::node) struct RebuildRequest<D: Direction> {
    ...
    pub(in crate::node) backend_properties: BackendPropertiesOf<D>,
    ...
    pub(in crate::node) shared: Weak<NodeShared<D>>,
}
...
pub(in crate::node) enum SwapOutcome<D: Direction> {
    ...
    Installed {
        dsp: D::Device,
        config: PortConfig,
        applied_buffer: backend::AppliedBufferGeometry,
    },
    ...
    // blocker on an exclusive device: retire it,
...
pub(in crate::node) struct RebuildWorkSlot<D: Direction> {
    pub(in crate::node) stopping: AtomicBool,
    pub(in crate::node) state: AtomicU8,
    value: UnsafeCell<Option<RebuildWork<D>>>,
...
impl<D: Direction> RebuildWorker<D> {
    ...
    pub(in crate::node) fn endpoint(&self) -> Arc<RebuildWorkSlot<D>> {
        self.work.clone()
    }
...
fn rebuild_worker_loop<D: Direction>(work: Arc<RebuildWorkSlot<D>>) {
    loop {
...
pub(in crate::node) struct RebuildContext<'a, D: Direction> {
    ...
    pub(in crate::node) shared: &'a Arc<NodeShared<D>>,
...
pub(crate) fn queue_rebuild<D: Direction>(state: &mut DataState<D>, port_idx: usize) -> bool {
    ...
    queue_port_rebuild(
        ...
        RebuildContext {
            ...
            backend_properties: state.backend_properties,
            log: &state.log,
            shared: &state.shared,
...
pub(in crate::node) struct DepositOnUnwind<D: Direction> {
    pub(in crate::node) shared: Arc<NodeShared<D>>,
    pub(in crate::node) port_idx: usize,
    pub(in crate::node) generation: u64,
        ...
        if !rebuild_request_is_current(&guard.shared, request.generation) {
            ...
            match res {
                Ok(outcome) => SwapOutcome::Installed {
                    ...
                    applied_buffer: outcome.applied_buffer,
                },
