...
const SOUND_MIXER_INFO: c_ulong = nix::request_code_read!(b'M', 101, std::mem::size_of::<MixerInfo>());
...
impl Mixer {
  ...
  pub fn open(unit: u32) -> Option<Self> {
  ...
  pub fn output_control(&self) -> Option<c_uint> {
    [SOUND_MIXER_VOLUME, SOUND_MIXER_PCM].into_iter().find(|d| self.has(*d))
  }
  ...
  pub fn level(&self, dev: c_uint) -> Option<(u32, u32)> {
  ...
  pub fn set_level(&self, dev: c_uint, left: u32, right: u32) -> bool {
    let v = left.min(100) | (right.min(100) << 8);
    self.write_int(dev, v as c_int)
  }

  pub fn muted(&self, dev: c_uint) -> Option<bool> {
  ...
  pub fn set_muted(&self, dev: c_uint, mute: bool) -> bool {
    let Some(mask) = self.read_int(SOUND_MIXER_MUTE) else {
      return false;
    };
    let bit  = 1u32 << dev;
    let mask = if mute { mask as u32 | bit } else { mask as u32 & !bit };
  ...
  pub fn modify_counter(&self) -> Option<c_int> {
